...
# create rc config
chmod +x tasks/deploy/backend-controller.yml.sh && tasks/deploy/backend-controller.yml.sh
...
if [ $($KUBERNETES_CMD get rc | grep -c backend) -ne 1 ]; then
    echo "Create backend rc with replicas:" $REPLICAS_NUMBER
    $KUBERNETES_CMD create -f tasks/deploy/backend-controller.yml
else
    echo "Rolling update authorizer rc"
    $KUBERNETES_CMD rolling-update backend --update-period=10s --image=${EXTERNAL_REGISTRY_ENDPOINT}/backend:${CIRCLE_BRANCH}.${CIRCLE_SHA1}
fi
